...
export default class SearchPage {
  ...
  async openSearchJobs() {
    return await this.page.getByRole("heading", {
      name: this.selectors.searchJobs,
    });
  }
  async openSearchCandidates() {
    return await this.page.getByRole("heading", {
      name: this.selectors.searchCandidates,
    });
  }
  async openCandidateKeywordSearch() {
    return await this.page.getByRole("heading", {
      name: this.selectors.candidateKeywordSearch,
    });
  }
  async openSavedSearch() {
    return await this.page.getByRole("heading", {
      name: this.selectors.savedSearch,
    });
  }
  async openSearchTalent() {
    return await this.page.getByRole("heading", {
      name: this.selectors.searchTalent,
    });
  }
  async openInternetAddressBookSearch() {
    return await this.page.getByRole("heading", {
      name: this.selectors.internalAddressBookSearch,
    });
  }
  ...
  async navigateToAllSearchLinks() {
    ...
    for (let i = 1; i < count; i++) {
      ...
      await this.page.waitForSelector(
        '[data-testid="jobs-page-left-menu-ul-list"]',
        { timeout: 5000 }
