...
test.beforeEach(async ({ page }) => {
  loginData = await ExcelUtils.readData("utils/JobtrainData.xlsx", "LoginData");
...
test("Jobtrain login test for user", async ({ page }) => {
  const login = new LoginPage(page); // Initialize LoginPage object
  await login.navigateToJobtrain("Login"); // Navigate to the login page
...
test("Jobtrain login test example", async ({ page }) => {
  const login = new LoginPage(page); // Initialize LoginPage object
  await login.navigateToJobtrain("Login"); // Navigate to the login page
  await login.enterUserName(loginData[0].Username); // Enter username
  await login.enterPassword(loginData[0]["Password"]); // Enter password
...
test.skip("Failed login with incorrect password ", async ({ page }) => {
  ...
  try {
    const invalidUserAndPassword = await (
      await login.invalidUserOrPasswordText()
    ).innerText();
...
test.skip("Jobtrain write data to Excel file ", async ({ page }) => {
  // Initialize LoginPage object
  ...
  await ExcelUtils.writeData(
    "D:\\JobTrain Documents\\JobtrainData.xlsx",
    "LoginData",
    data,
    {
...
test.skip("Jobtrain write data to Excel with columsn only ", async ({
  page,
}) => {
  // Initialize LoginPage object
  ...
  // Writing only specific columns with custom values
  await ExcelUtils.writeDataWithColumns(
    "D:\\JobTrain Documents\\JobtrainData.xlsx",
    "Sheet1",
    [],
    {
