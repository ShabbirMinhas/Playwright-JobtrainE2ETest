...
test('System settings main page verification', async ({ page }) => {
    const sysSettings = new SystemSettingsPage(page);
    (await sysSettings.navigateToSystemSettingsQuickLink());
    await page.waitForLoadState('networkidle');
    await sysSettings.verifySystemSettingsScreen();


});
test('System settings communication page verification', async ({ page }) => {
    const sysSettings = new SystemSettingsPage(page);
    (await sysSettings.systemSettingsLeftMenuLinkByName('Communication'));
    await page.waitForLoadState('networkidle');
    await sysSettings.verifySystemSettingsScreen();
