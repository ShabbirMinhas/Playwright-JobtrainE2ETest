...
test.beforeEach(async ({ page }) => {
    ...
    await page.goto('JobsAndTalents/Jobs/List', { waitUntil: 'domcontentloaded', timeout: 65000 });
...
test('navigate to add job link', async ({ page }) => {
    const jobsPage = new JobPage(page);
    await jobsPage.navigateToAddJobLink(page);
   
    
});
test('navigate to add to my job link', async ({ page }) => {
    const jobsPage = new JobPage(page);
    await jobsPage.navigateToAddJobLink(page);
    
});
test('navigate to edit job link', async ({ page }) => {
    const jobsPage = new JobPage(page);
    await jobsPage.navigateToEditJobs(page);
